...
async function getInlineKeyboardOpenListButton() {
	...
	return InlineKeyboard.url(
		...
		`https://t.me/${botInfo.username}?startapp=${BOT_HOME_GROUP_CHAT_ID}`
	);
}
...
bot.command('start', async (ctx) => {
	if (ctx.match === BOT_HOME_GROUP_CHAT_ID) {
	...
	if (ctx.inlineQuery.query === '') {
		await ctx.answerInlineQuery([], {
			button: {
				...
				start_parameter: BOT_HOME_GROUP_CHAT_ID
			},
			cache_time: 24 * 60 * 60
		});
	} else {
	...
	if (lastMessageId) {
		if (complete) {
			if (newList) {
				...
				await bot.api.editMessageReplyMarkup(BOT_HOME_GROUP_CHAT_ID, lastMessageId);
			}

			await bot.api.unpinChatMessage(BOT_HOME_GROUP_CHAT_ID, lastMessageId);
			...
			if (list.every((category) => !category.items.length)) {
				await bot.api.deleteMessage(BOT_HOME_GROUP_CHAT_ID, lastMessageId);
				await lastMessageIdService.deleteId();
				return;
			}

			// FIXME: causes 400: Bad Request: message is not modified - if checked more than one in short time
			await bot.api.editMessageText(BOT_HOME_GROUP_CHAT_ID, lastMessageId, shoppingList(list), {
				parse_mode: 'HTML',
...
export async function sendNewList() {
	...
	const list = await listService.getList();
	const { message_id } = await bot.api.sendMessage(BOT_HOME_GROUP_CHAT_ID, shoppingList(list), {
	...
	await bot.api.pinChatMessage(BOT_HOME_GROUP_CHAT_ID, message_id);
}
