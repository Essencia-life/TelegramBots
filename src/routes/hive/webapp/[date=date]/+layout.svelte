<script lang="ts">
	...
	import { getBookings } from '../calendar.remote';
	import ms from 'ms';
	...
	function prevDay() {
		...
		goto(resolve('/hive/webapp/[date=date]', { date }), {
			replaceState: true,
			noScroll: true
		});
	}
	...
	onMount(() => {
		preloadCode(resolve('/hive/webapp/[date=date]/booking', params));
	...
	afterNavigate((navigation) => {
		if (navigation.to?.route.id === '/hive/webapp/[date=date]') {
			...
			<div class="row-span-4 grid grid-rows-4 rounded-lg bg-white">
				{#each Array(4) as _, quarter}
					...
					<a
						...
						href={resolve('/hive/webapp/[date=date]/booking', {
							date: params.date
						}) +
							'?start=' +
							time}
	...
	<div class="column">
		{#each await getBookings(params.date) as booking (booking.id)}
			...
			<svelte:element
				...
				data-sveltekit-noscroll
				href={isOwn
					? resolve('/hive/webapp/[date=date]/booking/[[id]]', {
							date: params.date,
							id: booking.id!
						})
					: undefined}
...
<Button
	...
	onclick={() => goto(resolve('/hive/webapp/[date=date]/booking', params), { noScroll: true })}
>
	<Plus />
</Button>
