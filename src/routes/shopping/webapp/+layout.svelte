<script lang="ts">
	...
	import WebApp from '$lib/client/webapp';
	...
	onMount(async () => {
		...
		WebApp.SettingsButton?.show().onClick(() => {
			goto(resolve('/shopping/webapp/settings'));
		});
	...
	$effect(() => {
		WebApp.BackButton?.[page.route.id === '/shopping/webapp' ? 'hide' : 'show']();
	});
...
<svelte:boundary>
	...
	{#if ready}
		<main class="flex min-h-full flex-col gap-4">
			{#each await getList() as category (category.id)}
				<ListCategory {category} />
			{:else}
				<p>
					There are no lists available.<br />
					Go to <a href={resolve('/shopping/webapp/settings')}>settings</a> to create new lists.
				</p>
			{/each}
		</main>
